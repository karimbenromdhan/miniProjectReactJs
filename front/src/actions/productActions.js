...
export const addProductReview = (review, productid) => (dispatch, getState) => {
	...
	axios
		.post("http://localhost:4000/api/products/review", {
			review,
			productid,
			currentUser,
		})
...
export const editproduct = (productId) => (dispatch) => {
	dispatch({ type: "PUT_NEW_PRODUCT_REQUEST" });
	axios
		.put(`http://localhost:4000/api/products/${productId}`)
		.then((res) => {
			dispatch({ type: "PUT_NEW_PRODUCT_SUCCESS", payload: res.data });
			window.location.reload();
		})
		.catch((err) => {
			dispatch({ type: "PUT_NEW_PRODUCT_FAILED", payload: err });
		});
};
